...
export async function GET(
  ...
  try {
    ...
    // Security: Only allow internal/local access in development
    ...
    const versionPath = version ? version : "cv";

    let data, labels;
    if (lang === "en") {
      const enModule = await import(`@/app/en/${versionPath}/cv-data`);
      data = enModule.CV_DATA;
      labels = enModule.CV_LABELS;
    } else {
      const esModule = await import(`@/app/es/${versionPath}/cv-data`);
